...
const Results = () => {
    ...
    // Initialize allJobs from location.state.data or Redux store
    const allJobsFromStore = useSelector((state) => state.atsData.allJobs);
    const [allJobs, setAllJobs] = useState(location.state?.data || allJobsFromStore || []);
    const [filteredJobs, setFilteredJobs] = useState(allJobs);
    const [selectedJob, setSelectedJob] = useState(null);
    const [selectedJobID, setSelectedJobID] = useState(null);
    const [selectedJobTitle, setSelectedJobTitle] = useState(null);
    ...
    // Get selectedJobs from Redux store
    const reduxSelectedJobs = useSelector((state) => state.atsData.selectedJobs);
    const jobsInStore = useSelector((state) => state.atsData.jobs.map(job => job.id));

    // Initialize selectedJobs from Redux, filtered by allJobs
    const [selectedJobs, setSelectedJobs] = useState(() => {
        console.log("Initial reduxSelectedJobs:", reduxSelectedJobs);
        return reduxSelectedJobs.filter((job) =>
            allJobs.some((j) => j.link_no === job.link_no)
        );
    });

    // Sync local selectedJobs with Redux when reduxSelectedJobs or allJobs changes
    useEffect(() => {
        console.log("reduxSelectedJobs changed:", reduxSelectedJobs);
        const validSelectedJobs = reduxSelectedJobs
            .map((reduxJob) => allJobs.find((job) => job.link_no === reduxJob.link_no))
            .filter((job) => job !== undefined);
        console.log("Valid selectedJobs after mapping:", validSelectedJobs);
        setSelectedJobs(validSelectedJobs);
    }, [reduxSelectedJobs, allJobs]);

    // Log selectedJobs when it changes
    useEffect(() => {
        console.log("Local selectedJobs updated:", selectedJobs);
    }, [selectedJobs]);

    // Update Redux allJobs when location.state.data is available
    useEffect(() => {
        if (location.state?.data) {
            console.log("Updating allJobs from location.state.data:", location.state.data);
            setAllJobs(location.state.data);
            dispatch(updateAllJobs(location.state.data));
        }
    }, [location.state?.data, dispatch]);

    // Sync selectedJobs with allJobs when allJobs changes
    useEffect(() => {
        console.log("allJobs changed:", allJobs);
        const validSelectedJobs = selectedJobs.filter((job) =>
            allJobs.some((j) => j.link_no === job.link_no)
        );
        if (validSelectedJobs.length !== selectedJobs.length) {
            console.log("Updating selectedJobs due to allJobs change:", validSelectedJobs);
            setSelectedJobs(validSelectedJobs);
            dispatch(updateSelectedJobs(validSelectedJobs));
        }
        // Update filteredJobs to reflect allJobs
        setFilteredJobs(allJobs);
    }, [allJobs, selectedJobs, dispatch]);

    function clearAllJobsInREDUX() {
        dispatch(clearJobs()); // Only clear jobs, not allJobs or selectedJobs
    }
    ...
    useEffect(() => {
        console.log("Running fetchTitleDict and initialization");
        // Only call fetchTitleDict if allJobs is not empty
        if (allJobs.length > 0) {
            fetchTitleDict();
        } else {
            console.log("Skipping fetchTitleDict: allJobs is empty");
            setIsProcessing(false); // Avoid loading state if no jobs
        }
        clearAllJobsInREDUX();
        getNextJobID();
    }, [allJobs]);
    ...
    const fetchTitleDict = async () => {
        const jobTitleList = allJobs.map((job) => job.title);
        console.log("jobTitleList:", jobTitleList);
        // Skip API call if jobTitleList is empty
        if (jobTitleList.length === 0) {
            console.log("No job titles to fetch");
            setIsProcessing(false);
            return;
        }
        try {
            ...
            console.error("Error fetching jobTitles:", error);
            setIsProcessing(false); // Ensure UI doesn't hang
            setJobTitleDict({}); // Fallback to empty dict
            return { error: error.message };
        }
    };
    ...
    const handleCheckboxChange = (job) => {
        console.log("Checkbox toggled for job:", job.link_no);
        let updatedJobs;
        if (selectedJobs.some((j) => j.link_no === job.link_no)) {
            updatedJobs = selectedJobs.filter((j) => j.link_no !== job.link_no);
            dispatch(removeSelectedJob(job));
        } else {
            updatedJobs = [...selectedJobs, job];
            dispatch(addSelectedJob(job));
        }
        setSelectedJobs(updatedJobs);
        console.log("Updated selectedJobs:", updatedJobs);
    };

    const handleSelectAll = () => {
        if (filteredJobs.every((job) => selectedJobs.some((j) => j.link_no === job.link_no))) {
            dispatch(clearSelectedJobs());
            setSelectedJobs([]);
            console.log("Deselected all jobs");
        } else {
            dispatch(updateSelectedJobs([...filteredJobs]));
            setSelectedJobs([...filteredJobs]);
            console.log("Selected all filtered jobs:", filteredJobs);
        }
    };
    ...
    const handleSubmit = async () => {
        console.log("Submitting selectedJobs:", selectedJobs);
        await addSelectedJobsToStore(selectedJobs);
        setIsGenerating(false);
        navigate("/display");
    };
    ...
    async function getNextJobID() {
        const jobIDList = allJobs.map((job) => job.link_no);
        const maxID = jobIDList.length > 0 ? Math.max(...jobIDList) : 0;
        setNextJobID(maxID + 1);
    }
                    ...
                    {filteredJobs.length > 0 && (
                        <div className="mb-4">
                            <button
                                ...
                                onClick={handleSelectAll}
                            >
                                {filteredJobs.every((job) => selectedJobs.some((j) => j.link_no === job.link_no))
                                    ? "Deselect All"
                                    : "Select All"}
                            </button>
                        </div>
                            ...
                            <div key={job.link_no} className="flex items-start">
                                <label className="inline-flex items-center mt-4 mr-4 cursor-pointer">
                                    <input
                                        type="checkbox"
                                        className="form-checkbox h-5 w-5 bg-white border-2 border-gray-400 rounded focus:ring-black transition duration-200"
                                        checked={selectedJobs.some((j) => j.link_no === job.link_no)}
                                        onChange={() => handleCheckboxChange(job)}
                                    />
                                </label>
                                <div className="p-4 rounded-lg bg-gray-100 shadow-md flex-1 min-h-[100px] w-full">
