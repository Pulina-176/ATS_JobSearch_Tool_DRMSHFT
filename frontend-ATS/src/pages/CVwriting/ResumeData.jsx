...
const ResumeData = () => {
  ...
  const renderTabContent = () => {
      ...
      return (
        <div className="space-y-4">
          {data.map((item, index) => (
            <div
              key={index}
              className="p-4 px-6 bg-white rounded-lg shadow-sm border border-gray-200"
            >
    ...
    return (
      <div className="space-y-4">
        <div className="p-4 px-6 bg-white rounded-lg shadow-sm border border-gray-200">
  ...
  return (
    <div className="flex flex-col p-5 text-gray-900 bg-black bg-opacity-10 min-h-screen w-full overflow-x-auto">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-white px-4 pt-4">Resume Data</h1>
      </div>
      {resumeData && Object.keys(resumeData).length > 0 ? (
        <div className="flex flex-wrap border-b border-gray-300 mb-4 w-full">
          {Object.keys(resumeData).map((tab) => (
            <button
              key={tab}
              className={`px-4 py-2 font-semibold text-lg whitespace-nowrap ${
                activeTab === tab
                 ? "border-b-2 border-yellow-500 text-yellow-500" : "text-white hover:text-yellow-500"}
              }`}
              onClick={() => setActiveTab(tab)}
            >
              {tab     .split("_")
